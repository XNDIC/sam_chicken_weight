...
sys.path.append(os.path.abspath("../chicken_count_new/yolov7"))
print(sys.path)
import numpy as np
...
class BaseTracker(object):
    def __init__(self, opts, frame_rate=30, *args, **kwargs) -> None:
        ...
        self.NMS = False  # whether use NMS
    ...
    def update(self, det_results, ori_img):
        ...
        # det_results = scale_coords(self.model_img_size[:2], det_results[:,:4], ori_img.shape[:2])
        ...
        gain = min(384 / img_h, 640 / img_w)  # gain  = old / new
        pad = (640 - img_w * gain) / 2, (384 - img_h * gain) / 2
        det_results[:, [0, 2]] -= pad[0]  # x padding
        det_results[:, [1, 3]] -= pad[1]  # y padding
        det_results[:, :4] /= gain

        det_results[:, 0] = det_results[:, 0].clip(0, img_w)  # x1
        det_results[:, 1] = det_results[:, 1].clip(0, img_h)  # y1
        det_results[:, 2] = det_results[:, 2].clip(0, img_w)  # x2
        det_results[:, 3] = det_results[:, 3].clip(0, img_h)  # y2
        det_results = det_results.round()
